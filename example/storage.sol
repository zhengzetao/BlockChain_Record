...
contract FileHashStorage is Owned {
    ...
    */
    ...
    function upload(string personName, string fileHash, string fileName, uint fileSize,string operation) onlyOwner public {
        ownerID++;
        owners.push(personName);
        File memory f = File(operation,fileName, now, fileSize);
        files[fileHash] = f;
